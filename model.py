...
def get_ortho_polynomials(degrees: int, length: int, alpha=0, beta=0):
    """Generate Jacobi orthogonal polynomials evaluated on a discretized interval.

    This function computes a set of Jacobi polynomials of degrees 0 through `degrees`,
    evaluated at `length` evenly spaced points over the interval [0, 1]. The polynomials
    are computed using numerical integration of the polynomial coefficients.

    Args:
        degrees (int): Maximum polynomial degree. The function generates `degrees + 1`
            polynomials (degrees 0 through `degrees`), where degree 0 is the constant term.
        length (int): Number of discretization points for evaluating the polynomials
            over the interval [0, 1].
        alpha (float, optional): First shape parameter of the Jacobi polynomials.
            Must be > -1. Defaults to 0 (Legendre polynomials when beta=0).
        beta (float, optional): Second shape parameter of the Jacobi polynomials.
            Must be > -1. Defaults to 0 (Legendre polynomials when alpha=0).

    Returns:
        np.ndarray: Array of shape (degrees + 1, length) where each row contains
            one polynomial evaluated at the discretization points. Row i corresponds
            to the polynomial of degree i.
    """
    coeffs = np.vstack(
        [
            np.pad(np.flip(jacobi(degree, alpha, beta).coeffs), (0, degrees - degree))
...
class PleiadesLayer(nn.Conv3d):
    ...
    def _parameterize_weight(self, degree, alpha, beta):
        transform = get_ortho_polynomials(
            degree, self.kernel_size[-1], alpha=alpha, beta=beta
        )
    ...
    def resample(self, length):
        transform = get_ortho_polynomials(
            self.degree, length, alpha=self.alpha, beta=self.beta
        )
        transform = torch.tensor(transform).float()
        transform = transform / self.scale
...
class STBlock(nn.Module):
    ...
    def forward(self, input):
        if self.dws:
            x = F.relu(
                self.t_skip(input[..., self.t_kernel_size - 1 :]) + self.conv_t(input)
            )
        else:
            x = self.conv_t(input)
    ...
    def __init__(
        ...
        **kwargs
        ...
        for block_id in range(len(depthwises)):
            in_channels, med_channels, out_channels = channels[
                2 * block_id : 2 * block_id + 3
            ]
